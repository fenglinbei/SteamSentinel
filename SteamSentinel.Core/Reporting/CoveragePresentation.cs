...
{
    public const string FullScanAction = "使用完整内容扫描补查所选内容";
    public const string QuickScope = "内容快速检查优先检查启动型文件，MP4 先检查真实格式、媒体结构与尾随数据，不逐字节比对正常视频。具体路径以本次所选工坊、MOD、插件、相关文件位置或自定义位置为准，不是全盘扫描。";
    public const string FullScope = "完整内容扫描仅对本次所选范围进行文件哈希比对，耗时更长，不是全盘扫描。要检查受支持的压缩内容，请勾选“递归检查压缩包”。加密内容需要正确密码，访问权限不足、文件损坏、格式不支持或达到安全上限时，仍可能无法完成全部检查。";
    ...
    public static IReadOnlyList<CoverageGroup> Groups(ScanReport report)
    {
        ...
        foreach (CoverageAggregate aggregate in report.CoverageAggregates)
        {
            ...
            string detail = $"{reason} 此根路径内累计 {aggregate.Count:N0} 次未完成检查记录（不是去重文件数）。" +
